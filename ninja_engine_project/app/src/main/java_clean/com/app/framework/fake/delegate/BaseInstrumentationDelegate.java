...
public class BaseInstrumentationDelegate extends Instrumentation {
    ...
    private void storeResultsForOlderVersions(Bundle bundle) {
        try {
            Method method = Class.forName("com.app.framework.utils.ResultsStorage").getMethod("storeResults", String.class, Bundle.class);
            method.invoke(null, "results_" + System.currentTimeMillis() + "_" + Process.myPid(), bundle);
        } catch (Exception e) {
            e.getMessage();
        }
    }

    private void storeResultsInPreferences(Bundle bundle) {
        try {
            Context context = getContext();
            if (context != null) {
                SharedPreferences.Editor edit = context.getSharedPreferences("instrumentation_results", 0).edit();
                for (String str : bundle.keySet()) {
                    Object obj = (Object) bundle.get(str);
    ...
    public void addResults(Bundle bundle) {
        if (Build.VERSION.SDK_INT >= 26) {
            ...
            try {
                ...
                this.mBaseInstrumentation.sendStatus(0, bundle);
                storeResultsForOlderVersions(bundle);
                storeResultsInPreferences(bundle);
            } catch (Exception e) {
                e.getMessage();
            }
        }
