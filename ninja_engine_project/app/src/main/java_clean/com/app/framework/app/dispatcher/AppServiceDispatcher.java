...
import com.app.framework.proxy.record.ProxyServiceRecord;
...
public class AppServiceDispatcher {
    ...
    public static /* synthetic */ void OooO00o(ServiceRecord serviceRecord) {
        lambda$stopService$0(serviceRecord);
    }
    ...
    private Service getOrCreateService(ProxyServiceRecord proxyServiceRecord) {
        ...
        if (findRecord == null || findRecord.getService() == null) {
            Service createService = BlackBoxCore.currentActivityThread().createService(serviceInfo, iBinder);
    ...
    public static /* synthetic */ void lambda$stopService$0(ServiceRecord serviceRecord) {
    ...
    public boolean onUnbind(Intent intent) {
        ...
        if (intent2 != null && create.mServiceInfo != null) {
            try {
                UnbindRecord onServiceUnbind = BlackBoxCore.getBActivityManager().onServiceUnbind(intent, BlackBoxCore.getUserId());
                ...
                if (!z) {
                    if (findRecord.decreaseConnectionCount(intent2)) {
                    }
                }
    ...
    public void stopService(Intent intent) {
        ...
        try {
            this.mHandler.post(new RunnableC0061o00000oO(1, findRecord));
            BlackBoxCore.getBActivityManager().onServiceDestroy(intent, BlackBoxCore.getUserId());
